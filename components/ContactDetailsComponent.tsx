          ...
          <div key={i} style={{ marginBottom: 18 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <div className="float-container" style={{ flex: 1 }}>
                <input
                  type="text"
                  className={`float-input ${i === 0 && userData && (userData.fullName || userData.name) ? 'auto-filled' : ''}`}
                  placeholder=" "
                  value={item.value}
                  onChange={(e) =>
                ...
                <label className="float-label">
                  Contact Person *
                  {i === 0 && userData && (userData.fullName || userData.name) && " (Auto-filled)"}
                </label>
              </div>
              ...
              {i > 0 && (
                <img
                  src="/remove.svg"
                  onClick={() => removeField(setContactPersons, contactPersons, i)}
        ...
        {mobileNumbers.map((item, i) => (
          <div key={i} style={{ marginBottom: 18 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              >
                <img src="/india_flag.svg" style={{ width: 18 }} alt="India Flag" />
                <span>+91</span>
              </div>

              <div className="float-container" style={{ flex: 1 }}>
                <input
                  className={`float-input ${i === 0 && mobileNumber ? 'auto-filled' : ''}`}
                  placeholder=" "
                  value={item.value}
                  onChange={(e) => {
        ...
        {whatsappNumbers.map((item, i) => (
          <div key={i} style={{ marginBottom: 18 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              >
                <img src="/whatsapp.svg" style={{ width: 18 }} alt="WhatsApp" />
                <span>+91</span>
              </div>

              <div className="float-container" style={{ flex: 1 }}>
                <input
                  className={`float-input ${i === 0 && sameAsMobile ? 'auto-filled' : ''}`}
                  placeholder=" "
                  value={item.value}
                  disabled={sameAsMobile}
                  onChange={(e) =>
                    updateField(setWhatsappNumbers, whatsappNumbers, i, e.target.value)
                  }
                  required
                />
                <label className="float-label">
              ...
              {!sameAsMobile && i > 0 && (
                <img
                  src="/remove.svg"
                  onClick={() => removeField(setWhatsappNumbers, whatsappNumbers, i)}
                  style={{ width: 18, height: 18, cursor: "pointer" }}
                  alt="Remove"
                />
              )}
